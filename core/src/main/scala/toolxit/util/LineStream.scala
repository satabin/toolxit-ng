/** Taken from https://github.com/djspiewak/gll-combinators */
 ...
 */
sealed abstract class LineStream(val line: String, val lineNum: Int, val colNum: Int)
    extends LinearSeq[Char]
    with LinearSeqLike[Char, LineStream]
    with CharSequence { outer =>

  override def newBuilder: Builder[Char, LineStream] =
    new ArrayBuffer[Char]().mapResult(buffer => LineStream(buffer: _*))
  ...
  def !::(head: Char): LineStream =
    new LazyLineCons(head, this, line, lineNum, colNum)
...
object !:: {
  ...
  def unapply(stream: LineStream): Option[(Char, LineStream)] =
    if (stream.isEmpty)
      None
    else
